...
def gitpartner_source_root(root: Path) -> Path:
    ...
    source_root = root.resolve() / product / "src"
    marker = (
        source_root
        ...
        contract.update(
            {
                ...
                "performance_measurement_mode": "fast-single",
                ...
                "correctness_evidence_mode": (
                    ...
                    if execution_profile == FUSED_SCALABLE_PROFILE
                    and correctness_repetitions == 1
                    else "standalone-batched-process"
                ),
            }
        )
    return contract
...
def legacy_test_shell(
    ...
    *,
    ...
    from limited_remote_partner.gateway.submit_job import _build_job
...
def staged_test_shells(
    ...
    *,
    ...
    fragments = list(_b_local_smoke_fragments(args))
    try:
...
def build_case_cache_requirement(
    ...
    *,
    ...
    generator_source = (
        gitpartner_source_root(Path(__file__).resolve().parents[5])
        / "limited_remote_partner"
        / "resources"
        / "case_cache.py"
    )
...
def source_cann_environment_snippet() -> str:
    package_root = gitpartner_source_root(Path(__file__).resolve().parents[5])
    if str(package_root) not in sys.path:
        sys.path.insert(0, str(package_root))
    from limited_remote_partner.gateway.submit_job import _source_cann_env_snippet

    return _source_cann_env_snippet()
