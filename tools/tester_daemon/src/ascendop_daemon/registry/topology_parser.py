...
from ascendop_daemon.registry.models import (
    BackendEndpoint, ExecutionEnvironment, ExecutionNode,
...
def parse_route_endpoint(
    ...
    *,
    ...
    management = object_value(row, "management_binding")
    management_mode = str(management.get("mode") or "")
    ...
    if management_mode:
        if management_mode != "direct-ssh":
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} has invalid management binding "
                f"mode {management_mode!r}"
            )
        if not re.fullmatch(r"[A-Za-z0-9._-]+", management_ssh_alias):
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} management ssh_alias must be a "
                "non-empty safe token"
            )
        if (
            not management_remote_gitpartner_root.startswith("/")
            or "\x00" in management_remote_gitpartner_root
            or "\n" in management_remote_gitpartner_root
            or "\r" in management_remote_gitpartner_root
        ):
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} management remote_gitpartner_root "
                "must be an absolute POSIX path"
            )
        expected_prefix = environment.remote_root.rstrip("/") + "/"
        if not management_remote_gitpartner_root.startswith(expected_prefix):
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} management remote_gitpartner_root "
                f"must be below environment remote_root {environment.remote_root}"
            )
        if (
            not management_remote_runtime_root.startswith("/")
            or "\x00" in management_remote_runtime_root
            or "\n" in management_remote_runtime_root
            or "\r" in management_remote_runtime_root
        ):
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} management remote_runtime_root "
                "must be an absolute POSIX path"
            )
        if not management_remote_runtime_root.startswith(expected_prefix):
            raise SystemRegistryError(
                f"route endpoint {endpoint_id} management remote_runtime_root "
                f"must be below environment remote_root {environment.remote_root}"
            )
    elif (
        management_ssh_alias
        or management_remote_gitpartner_root
        or management_remote_runtime_root
    ):
        raise SystemRegistryError(
    ...
    normalized = {
        ...
        "management_ssh_alias": management_ssh_alias,
        "management_remote_gitpartner_root": management_remote_gitpartner_root,
        "management_remote_runtime_root": management_remote_runtime_root,
    }
    identity = {
        **normalized,
