...
class RequestRoutingRepository:
    ...
    def create_test_request(
        ...
        registration_generation = str(
            manifest.get("registration_generation") or ""
        )
        requirements = manifest.get("execution_requirements", {})
        if not all(
            (request_id, request_digest, operator_id, test_version, registration_generation)
    ...
    def route_test_request(
        ...
        with self.transaction() as conn:
            ...
            if endpoint_row is None or not endpoint_row[1] or endpoint_row[2] or not endpoint_row[3]:
                raise ControlDatabaseError(
            ...
            outbox_payload = {
                ...
                "operator_registration_generation": request[
                    "registration_generation"
                ],
                ...
                "origin_workspace": str(
                    task_profile_info.get("origin_workspace", "")
                ),
            }
