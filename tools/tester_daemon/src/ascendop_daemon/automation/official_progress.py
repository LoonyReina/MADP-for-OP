...
class OfficialProgressPublisher:
    ...
    def _candidate(
        ...
        source_root = self._root_path(manifest["payload_sources"]["source_snapshot"])
    ...
    def _lineage_status(self, path: Path, actual_digest: str) -> tuple[bool, str]:
        ...
        if not resolved_parent.is_dir() or canonical_tree_digest(resolved_parent) != parent_digest:
