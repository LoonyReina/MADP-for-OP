...
class GitPartnerNodeAdmissionReconciler:
    ...
    def __init__(
        ...
        *,
        ...
        trusted_lease_probe_timeout_seconds: int = 30,
        node_ack_delivery_timeout_seconds: int = 60,
        ...
        self.trusted_lease_probe_timeout_seconds = max(
            10,
            min(120, int(trusted_lease_probe_timeout_seconds)),
        )
        self.node_ack_delivery_timeout_seconds = max(
            15,
            min(180, int(node_ack_delivery_timeout_seconds)),
        )
                    ...
                    try:
                        ...
                        probe = self.adapter_factory(endpoint).snapshot(
                            ...
                            wait_timeout_seconds=(
                                self.trusted_lease_probe_timeout_seconds
                            ),
                        )
            ...
            try:
                with NamedProcessLock(
                    ...
                    result = self.adapter_factory(endpoint).acknowledge_node(
                        ...
                        ack_path=ack_path,
                        wait_timeout_seconds=(
                            self.node_ack_delivery_timeout_seconds
                        ),
                    )
                    _write_json_atomic_if_changed(
                        marker_path,
