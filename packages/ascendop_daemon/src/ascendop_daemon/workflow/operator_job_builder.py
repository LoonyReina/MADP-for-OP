...
from ascendop_daemon.workflow.operator_job_files import (
    copy_tree_without_symlinks,
    engine_build_dir_name,
    filesystem_path,
    gitpartner_vendor,
    option_value,
    release_name,
...
from ascendop_daemon.workflow.runtime_boundary_overlay import (
    NATIVE_WORKSPACE_QUERY_ATTRIBUTION_FLAG,
    TRACE_FLAG as RUNTIME_BOUNDARY_TRACE_FLAG,
    apply_runtime_boundary_overlay,
)
from ascendop_daemon.workflow.runtime_compatibility import (
    runtime_compatibility_fragments,
...
def build_performance_runtime_budget(
    ...
    *,
    ...
    case_count = max(1, int(test_contract.get("performance_case_count", 0) or 0))
    expected_rows = max(
    ...
    historical_budget = math.ceil(max(historical_p95 * 1.5, historical_max * 1.25))
...
def apply_stage_timeout_budgets(
    ...
    *,
    ...
    profiler_process_count = (
        2 if profiler_mode in {"deep-dual", "batched-primary-roofline"} else 1
    )
    ...
    performance_timeout = int(performance_budget.get("stage_timeout_seconds", 0) or 0)
    defaults = {
...
def build_compatibility_job(
    ...
    *,
    ...
    runtime_compatibility = validate_runtime_compatibility(runtime_compatibility)
    op = parsed["op"]
    runtime_operator_name = str(runtime_operator_name or op).strip()
    if not runtime_operator_name:
        raise EngineJobBuildError("runtime operator name is empty")
    ...
    if submit_root != root and root not in submit_root.parents:
        raise EngineJobBuildError(
            f"engine submit root escapes workspace: {submit_root}"
        )
    sources = {
    ...
    if profiler_evidence:
        ...
        for component_name in (
            "profiler_evidence_runner.py",
            "profiler_row_attribution.py",
        ):
            component_source = workflow_root / component_name
            if not component_source.is_file():
                raise EngineJobBuildError(
                    f"profiler component missing: {component_source}"
                )
            shutil.copy2(component_source, payload_root / component_name)
    ...
    if case_cache_prewarm:
        ...
        optional_artifacts = [
            ...
            "result/OPERATOR_INSTALL_PRECHECK.txt",
            "result/RUNTIME_READINESS.json",
            "result/OFFICIAL_TEMPLATE_SYNC.json",
    ...
    if runtime_compatibility:
        required_artifacts.append("result/RUNTIME_COMPATIBILITY.json")
    else:
        optional_artifacts.append("result/RUNTIME_COMPATIBILITY.json")
    if RUNTIME_BOUNDARY_TRACE_FLAG in runtime_compatibility:
        required_artifacts.extend(
            [
    ...
    if NATIVE_WORKSPACE_QUERY_ATTRIBUTION_FLAG in runtime_compatibility:
    ...
    spec = {
        ...
        "scheduler_policy": {
            "queue_preactivation": ("enabled" if queue_preactivation else "disabled"),
            ...
            "device_continuation": ("enabled" if device_continuation else "disabled"),
        },
        ...
        spec["input_identity"].update(
            {
                ...
                "profiler_blocker_generation": normalized_plan["blocker_generation"],
        ...
        spec["input_identity"].update(
            {
                ...
                "performance_capture_mode": test_contract["performance_capture_mode"],
                "case_cache_protocol": test_contract["case_cache_protocol"],
                "correctness_evidence_mode": test_contract["correctness_evidence_mode"],
    ...
    *,
    ...
    if meta_path.is_file():
        try:
            ...
            raise EngineJobBuildError(
                f"cannot read engine case metadata: {meta_path}: {exc}"
            ) from exc
        if not isinstance(raw, dict):
            raise EngineJobBuildError(
                f"engine case metadata must be an object: {meta_path}"
            )
        meta = raw
    ...
    performance_score_groups = performance_score_group_contract(meta, performance_cases)
    contract = {
        "protocol_version": (
...
def normalize_profiler_plan(
    ...
    *,
    ...
    required_strings = (
        ...
        "request_sha256",
        "request_state_path",
        "profiler_execution_contract_digest",
        "target_version",
        "target_source_sha256",
    )
    ...
    profiler_kernel_name = str(plan.get("profiler_kernel_name") or "").strip()
    if not profiler_kernel_name:
        raise EngineJobBuildError("profiler plan is missing profiler_kernel_name")
    ...
    if any(case_id not in available for case_id in cases):
        raise EngineJobBuildError(f"profiler plan cases escape test contract: {cases}")
    if any(case_id not in cases for case_id in roofline_cases):
        raise EngineJobBuildError(
            "profiler roofline cases must be a subset of primary cases"
        )
    requested_mode = str(
        plan.get("profiler_mode") or plan.get("collection_mode") or "fast-single"
    )
    mode_aliases = {
    ...
    if profiler_mode is None:
        raise EngineJobBuildError(f"unsupported profiler mode: {requested_mode}")
    if profiler_mode == "deep-dual" and roofline_cases != cases:
        raise EngineJobBuildError(
            "deep-dual requires roofline cases to match all primary cases"
        )
    if profiler_mode == "fast-single" and roofline_cases:
        raise EngineJobBuildError("fast-single does not accept roofline cases")
    ...
    requested_timeout_seconds = int(
        plan.get("profile_timeout_seconds") or REGULAR_PROFILE_PROCESS_TIMEOUT_SECONDS
    )
    if (
    ...
    if not metrics or not re.fullmatch(r"[A-Za-z0-9_,.-]+", metrics):
        raise EngineJobBuildError(f"unsafe profiler primary metrics: {metrics!r}")
...
def performance_score_group_contract(
    ...
    for raw_name, raw_group in raw_groups.items():
        ...
        groups[name] = {
            ...
            "case_ids": group_ids,
            "weights": {
                f"case{case_id}": weight for case_id, weight in zip(group_ids, weights)
            },
        }
...
def staged_test_shells(
    ...
    *,
    ...
    runtime_compatibility = validate_runtime_compatibility(runtime_compatibility)
    compatibility_fragments = runtime_compatibility_fragments(runtime_compatibility)
    compatibility_audit = json.dumps(
        {
            "schema": "ascendop.runtime-compatibility.v1",
            "flags": list(runtime_compatibility),
            "environment": {
                "IGNORE_INFER_ERROR": (
                    "1"
                    if "ge-fixed-output-dtype-fallback" in runtime_compatibility
                    else ""
                )
            },
            "correctness": {
                "custom_op_profile_rounds": (
                    1
                    if "correctness-single-round-custom-op" in runtime_compatibility
                    else None
                )
            },
        },
    ...
    prepare = [
        item for item in fragments[:device_index] if not item.startswith("trap '")
    ]
    ...
    prepare = rewrite_wheel_cache_fragments(
        ...
        performance_samples_per_case=int(contract["performance_samples_per_case"]),
        expected_profile_round_anchor_count=int(
            contract.get("profile_round_anchor_count", 0) or 0
        ),
    )
    prepare = [
        rewrite_wheel_install_fragment(item, cached_site=cache_site) for item in prepare
    ]
    if execution_profile == FUSED_SCALABLE_PROFILE:
    ...
    prepare_common.extend(
        [
            ...
            (
                "printf '%s\\n' "
                + shlex.quote(compatibility_audit)
                + ' | tee "$RUN_DIR/RUNTIME_COMPATIBILITY.json" '
                + '> "$ASCENDOP_ENGINE_JOB_ROOT/result/RUNTIME_COMPATIBILITY.json"'
            ),
        ]
    )
    ...
    device.extend(compatibility_fragments)
    ...
    finalize.extend(
        [
            ...
            (
                ...
                '"$ASCENDOP_ENGINE_JOB_ROOT/result/$ARTIFACT" "$RUN_DIR/$ARTIFACT"; '
                "fi; done"
            ),
    ...
    if execution_profile in {
        ...
        correctness.extend(compatibility_fragments)
        ...
        capture.extend(compatibility_fragments)
        ...
        if execution_profile in SCALABLE_PROFILES:
            cached_cases = sorted(
                set(contract["correctness_cases"]) | set(contract["performance_cases"])
            )
...
def profiler_evidence_shell(plan: dict[str, Any]) -> str:
    ...
    commands.extend(
        [
            ...
            'test -f "$ASCENDOP_ENGINE_PAYLOAD_ROOT/profiler_row_attribution.py"',
...
def _gp_submit_module() -> Any:
    ...
    return importlib.import_module("limited_remote_partner.gateway." + "submit_job")
...
def case_cache_prewarm_stage_specs(
    ...
    if (
        "limited_remote_partner.resources.case_cache prepare"
        not in shells["runtime_wheel"]
    ):
        raise EngineJobBuildError(
            "case-cache prewarm runtime shell has no cache prepare"
        )
    finalize = "; ".join(
        [
...
def scalable_stage_specs(
    ...
    *,
    ...
    case_cache_fused = (fused_case_cache or fused_correctness) and not str(
        shells.get("case_cache") or ""
    )
    ...
    correctness = next(stage for stage in stages if stage["name"] == "correctness")
    performance = next(
...
def rewrite_wheel_install_fragment(fragment: str, *, cached_site: bool = False) -> str:
...
def rewrite_wheel_cache_fragments(
    ...
    *,
    ...
    site_args = (
        '--site-output "$RUN_DIR/py_site" --site-log "$RUN_DIR/pip_install.log" '
        if materialize_site
        else ""
    )
    cached_build = [
    ...
    if runtime:
        commands.extend(
            [
                ...
                'export LD_LIBRARY_PATH="$OPPROTO_LIB_DIR:$OPAPI_LIB_DIR:${LD_LIBRARY_PATH:-}"',
