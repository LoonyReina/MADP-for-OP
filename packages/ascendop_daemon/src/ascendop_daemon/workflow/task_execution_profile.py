...
def task_execution_profile_path(
    root: Path,
    ...
    return workspace_root / TASK_EXECUTION_PROFILE_NAME
...
def ensure_task_execution_profile(
    ...
    workspace = _object(registration.get("workspace"), "workspace")
    source = _relative_path(workspace.get("source"), "workspace.source")
    path = task_execution_profile_path(root, registration)
...
def build_task_execution_profile(
    ...
    definition = _object_or_empty(registration.get("definition"))
    runtime_operator_name = str(
        definition.get("runtime_operator_name")
        or registration.get("display_name")
        or registration.get("operator_id")
        or ""
    ).strip()
    if not runtime_operator_name:
        raise TaskExecutionProfileError("runtime operator name is empty")
    return {
        ...
        "extensions": {
            "ascendop.profiler/operator_name": runtime_operator_name,
        },
    }
...
def _validate_registration_identity(
    ...
    if profile.runtime_operator_name != expected_runtime_name:
        raise TaskExecutionProfileError(
            "task profile runtime operator identity is stale"
        )
