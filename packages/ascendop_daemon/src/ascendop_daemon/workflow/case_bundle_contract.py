...
def validate_case_bundle_contract(
    ...
    *,
    ...
    binding_path = task_case / "CASE_BINDING.json"
    if binding_path.is_file():
        try:
            ...
            return _result(
                False,
                f"task-case CASE_BINDING.json is invalid: {exc}",
                op,
                case_version,
                len(cases),
                cases_sha256,
        ...
        expected_binding = {
            "schema": "ascendop.task-case-binding.v1",
            "operator": op,
        ...
        if not isinstance(binding, dict) or any(
            ...
            return _result(
                False,
                "task-case CASE_BINDING.json does not identify the active case bundle",
                op,
                case_version,
                len(cases),
                cases_sha256,
        ...
        bound_case_dir = Path(str(binding.get("case_dir") or "")).as_posix()
        if not bound_case_dir or not case_dir.as_posix().endswith(
            "/" + bound_case_dir.lstrip("/")
        ):
            return _result(
                False,
                "task-case CASE_BINDING.json case_dir does not match the active case bundle",
                op,
                case_version,
                len(cases),
                cases_sha256,
    ...
    if run_validator and validator.is_file():
        ...
        env["ASCENDOP_ACTIVE_CASE_DIR"] = str(case_dir)
        env["ASCENDOP_ACTIVE_CASE_VERSION"] = case_version
        env["ASCENDOP_ACTIVE_OPERATOR"] = op
        try:
