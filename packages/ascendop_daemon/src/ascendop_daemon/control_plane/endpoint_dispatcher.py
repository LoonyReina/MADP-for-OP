...
from ascendop_daemon.control_plane.postprocess_dispatch import (
    dispatch_postprocess_recoveries,
)
from ascendop_daemon.control_plane.result_reconciler import (
    ResultProjectionCollector,
    reconcile_results,
)
...
class EndpointDispatcher:
    def __init__(
        ...
        *,
        ...
        self.max_postprocess_dispatch_attempts = max(
            1, int(max_postprocess_dispatch_attempts)
        )
        self.postprocess_dispatch_retry_seconds = max(
            1, int(postprocess_dispatch_retry_seconds)
        )
        ...
        self.query_owner_generation = str(query_owner_generation)
    ...
    def _run_serial(
        ...
        *,
        ...
        self._dispatch_postprocess_recoveries(report)
    ...
    def _query_payload(self, row: dict[str, Any]) -> dict[str, Any]:
        query_sequence = self.database.reserve_transport_query(
            str(row["outbox_id"]),
            owner_generation=self.query_owner_generation,
        )
        return {
            **dict(row["payload"]),
    ...
    def _acknowledge_pending(self, report: dict[str, Any]) -> None:
        ...
        for returned in rows:
            ...
            try:
                ...
                if (
                    self.result_collector is not None
                    and str(returned.get("state") or "") == "received"
                ):
                    projection = self.result_collector.collect(
                        dict(outbox),
                        dict(returned),
                    )
                    returned = self.database.record_transport_projection(
                        return_id,
                        projection,
                    )
                elif str(returned.get("state") or "") == "received":
                    returned = self.database.record_transport_projection(
                        return_id,
                        {
                            "state": "skipped-no-result-collector",
                            "stage_evidence": {},
                            "ingest_disposition": "ready",
                        },
                    )
                else:
                    stored = returned.get("projection")
                    projection = dict(stored) if isinstance(stored, dict) else None
                if str(returned.get("state") or "") == "recovery-decision-pending":
                    continue
                if (
                    str(returned.get("state") or "") == "recovery-unrecoverable"
                    and self.result_collector is not None
                    and isinstance(projection, dict)
                ):
                    destination = str(projection.get("destination") or "")
                    if destination:
                        projection["workflow_release"] = (
                            self.result_collector.release_hold(
                                destination,
                                reason=str(returned.get("hold_reason") or ""),
                            )
                        )
    ...
    def _dispatch_postprocess_recoveries(
        self,
        report: dict[str, Any],
    ) -> None:
        dispatch_postprocess_recoveries(
            ...
            max_dispatch_attempts=self.max_postprocess_dispatch_attempts,
            retry_delay_seconds=self.postprocess_dispatch_retry_seconds,
        )
...
class EndpointDispatcherPool:
    def __init__(
        ...
        *,
        ...
        result_collector: ResultProjectionCollector | None = None,
    ) -> None:
        self.dispatchers = list(dispatchers)
        self.database = (
            database
            if database is not None
            else dispatchers[0].database
            if dispatchers
            else None
        )
        self.result_collector = result_collector
