...
class TransportRepository:
        ...
        *,
        ...
        with self.transaction() as conn:
            ...
            conn.execute(
                ...
                "ELSE query_inflight_sequence END, "
                "query_inflight_owner_generation=CASE WHEN ?=1 THEN '' "
                "ELSE query_inflight_owner_generation END, accepted_at=?, error=?, "
                "updated_at=? "
                "WHERE outbox_id=?",
                (
                    status,
        ...
        with self.transaction() as conn:
            ...
            if row["state"] == "uncertain":
                conn.execute(
                    ...
                    "poll_attempts=0, query_inflight_sequence=0, "
                    "query_inflight_owner_generation='', error='', "
                    "updated_at=? "
                    "WHERE outbox_id=?",
        ...
        *,
        ...
        with self.transaction() as conn:
            ...
            conn.execute(
                ...
                "ELSE query_inflight_sequence END, "
                "query_inflight_owner_generation=CASE WHEN ?=1 THEN '' "
                "ELSE query_inflight_owner_generation END, updated_at=? "
                "WHERE outbox_id=?",
                (
                    attempts,
                    now,
    ...
    def reserve_transport_query(
        self,
        outbox_id: str,
        *,
        owner_generation: str = "",
    ) -> int:
        """Reserve one durable query identity per live consumer generation."""
        self.initialize()
        now = utc_now()
        with self.transaction() as conn:
            row = conn.execute(
            ...
            prior_owner = str(row["query_inflight_owner_generation"] or "")
            if sequence > 0 and owner_generation and prior_owner != owner_generation:
                sequence = 0
                conn.execute(
                    "UPDATE transport_outbox SET query_inflight_sequence=0, "
                    "query_inflight_owner_generation='', updated_at=? "
                    "WHERE outbox_id=?",
                    (now, outbox_id),
                )
                self._event(
                    conn,
                    "transport-query-owner-generation-changed",
                    "transport-outbox",
                    outbox_id,
                    {
                        "previous_owner_generation": prior_owner,
                        "owner_generation": owner_generation,
                        "superseded_query_sequence": int(
                            row["query_inflight_sequence"] or 0
                        ),
                    },
                )
            ...
            if sequence <= 0:
                ...
                conn.execute(
                    ...
                    "query_inflight_owner_generation=?, updated_at=? "
                    "WHERE outbox_id=?",
                    (sequence, sequence, owner_generation, now, outbox_id),
                )
                self._event(
                    conn,
