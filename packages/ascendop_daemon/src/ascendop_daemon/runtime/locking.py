...
class DaemonLock:
    ...
    def acquire(self) -> None:
        ...
        try:
            os.write(self.fd, _owner_metadata_line().encode("utf-8"))
...
class NamedProcessLock:
    ...
    def _register_waiter(self, deadline: float) -> None:
        ...
        for sequence in range(100):
            ...
            try:
                ...
                if not payload["start_token"]:
                    raise RuntimeError("process lock waiter identity is unavailable")
                os.write(
                    fd,
    ...
    def _cleanup_stale_waiters(self) -> None:
        ...
        for ticket_path in self._ordered_waiters():
            ...
            token = str(payload.get("start_token") or "")
            recorded_boot = str(payload.get("boot_id") or "")
            remove = (
                pid <= 0
                or not token
                or not recorded_boot
                or recorded_boot != boot_id()
                or not process_identity_matches(pid, token)
            )
            remove = remove or (
...
def lock_owner_alive(path: Path) -> bool:
    ...
    try:
        ...
        return False
    expected_token = str(metadata.get("start_token") or "")
    expected_boot = str(metadata.get("boot_id") or "")
    if (
        pid <= 0
        or not expected_token
        or not expected_boot
        or expected_boot != boot_id()
    ):
        return False
    return process_identity_matches(pid, expected_token)


def _owner_metadata_line() -> str:
    pid = os.getpid()
    token = process_start_token(pid)
    if not token:
        raise RuntimeError("process lock owner identity is unavailable")
    started_at_epoch = process_started_at_epoch(pid)
    started_at_field = (
    ...
    return (
        f"pid={pid} start_token={token} boot_id={boot_id()} "
        f"acquired_at={utc_now_iso()}{started_at_field}\n"
    )
