...
def load_active_transport_runtime(root: Path) -> ActiveTransportRuntime:
    ...
    if str(active.get("schema") or "") != "ascendop.active-release.v4":
        raise OSError("active Flow V4 release has an unsupported schema")
...
def _read_object(path: Path) -> dict[str, Any]:
    try:
        ...
        raise OSError(f"active Flow V4 release is unavailable: {path}") from exc
    if not isinstance(value, dict):
        raise OSError("active Flow V4 release must be a JSON object")
    return value
...
def _digest(value: dict[str, Any], field: str) -> str:
    ...
    if len(digest) != 64 or any(ch not in "0123456789abcdef" for ch in digest):
        raise OSError(f"active Flow V4 release has invalid {field}")
    return digest
