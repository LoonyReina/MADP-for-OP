...
def build_diagnostic_request(
    ...
    *,
    ...
    performance_cases = [
        int(value)
        for value in internal_spec.get("test_contract", {}).get("performance_cases", [])
    ]
    ...
    if gate_evidence:
        try:
            ...
            raise FlowV3RequestBuildError(
                "diagnostic budget gate evidence must be canonical JSON"
            ) from exc
        if not isinstance(parsed_gate_evidence, dict):
            raise FlowV3RequestBuildError(
                "diagnostic budget gate evidence must be an object"
            )
    ...
    envelope = build_envelope(
        ...
        execution={
            ...
            **(
            ...
            "stages": stages,
            "device_stage_budget": device_stage_budget_ledger(
                stages,
                granted_device_session_seconds=grant.granted_device_session_seconds,
            ),
...
def translate_diagnostic_stage_plan(
    ...
    *,
    ...
    for raw in raw_stages:
        ...
        if not name:
            raise FlowV3RequestBuildError(f"unsupported diagnostic stage: {original}")
        ...
        stages.append(
            {
                ...
                "failure_policy": (
                    "run-after-failure-evidence"
                    if name == "result-assemble"
                    else "skip-dependents"
                ),
                "run_after_failure": name == "result-assemble",
                "failure_depends_on": (
                    dependencies if name == "result-assemble" else []
                ),
...
def build_candidate_request(
    ...
    *,
    ...
    if operation_kind not in {
        ...
        raise FlowV3RequestBuildError(
            f"candidate request builder cannot build {operation_kind}"
        )
    diagnostic_correctness = operation_kind == "diagnostic-correctness-replay"
    if diagnostic_correctness and (publish_eligible or profiler_mode != "none"):
        raise FlowV3RequestBuildError(
            "diagnostic correctness replay must be non-publishable and correctness-only"
        )
    if diagnostic_correctness and not isinstance(diagnostic_plan, Mapping):
        raise FlowV3RequestBuildError(
            "diagnostic correctness replay requires a typed diagnostic plan"
        )
    requested_diagnostic_artifacts = {
        str(item)
        for item in (
            dict(diagnostic_plan or {}).get("requested_artifacts", [])
    ...
    internal_spec_path, payload_root = build_compatibility_job(
        ...
        attempt_index=attempt_index,
        workflow_ingest=not diagnostic_correctness,
        runtime_compatibility=effective_runtime_compatibility,
        runtime_operator_name=runtime_operator_name,
    )
    internal_spec = read_object(internal_spec_path)
    material = request_identity_material(
    ...
    if gate_evidence:
        ...
        if not isinstance(parsed_gate_evidence, dict):
            raise FlowV3RequestBuildError("heavy gate evidence must be a JSON object")
        gate_evidence_value = parsed_gate_evidence
        ...
        stages.append(
            {
                ...
                "failure_policy": (
                    ...
                    else "business-terminal-continue-cases"
                    if name == "correctness"
                    else "skip-dependents"
                ),
                ...
                "failure_depends_on": (
                    ["correctness"]
                    if run_after_failure and "correctness" in translated_names.values()
                    else []
                ),
                "max_stage_retries": (
                    1
                    if resource_class in {"host-light", "host-build-heavy", "export"}
                    else 0
                ),
            }
        )
